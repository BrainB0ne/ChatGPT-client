...
- `preload.js` exposes only `window.chatgptDesktop.saveMarkdown`, `window.chatgptDesktop.savePdf`, and `window.chatgptDesktop.printConversation`; the current UI injection lives in `main.js` via `webContents.executeJavaScript`.
