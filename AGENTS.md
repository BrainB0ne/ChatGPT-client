...
- `main.js` acquires a single-instance lock that restores the existing window on a second launch; it also creates the BrowserWindow, application menu, and Tray, loads `https://chatgpt.com`, limits in-window navigation to ChatGPT, OpenAI authentication, and official identity-provider sign-in pages, keeps allowed popup navigation in the existing window, and opens other URLs via `shell.openExternal`; it handles F5 reload and F12 DevTools, hides to tray on window close when enabled, clears ChatGPT session data from the File menu, persists tray, window-state, compatibility, voice/microphone, and export settings in Electron's `userData/settings.json`, configures explicit media permission handlers, and injects the Markdown export, HTML export, PDF export, print, and refresh buttons on `dom-ready`; exports use the active chat title and default to `chatgpt-export-YYYY-MM-DDTHH-MM-SS.md`, `.html`, or `.pdf` using local time.
- Remember Window Size and Position is an opt-in persisted setting. Save normal window bounds on move/resize, validate them against connected displays before restoring, and clear them when the setting is disabled.
...
- `preload.js` exposes only `window.chatgptDesktop.getExportPreferences`, `window.chatgptDesktop.getWindowTitle`, `window.chatgptDesktop.saveMarkdown`, `window.chatgptDesktop.saveHtml`, `window.chatgptDesktop.savePdf`, and `window.chatgptDesktop.printConversation`; the current UI injection lives in `main.js` via `webContents.executeJavaScript`.
